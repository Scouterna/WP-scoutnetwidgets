...
class ScouternaPlugins_ScoutnetWidgets_LedarepaAvdelning_Widget extends WP_Widget {
	public function __construct() {
		parent::__construct('scouternaplugins_ledarepaavdelning_widget',__( 'Visar ledare p&aring; en avdelning', 'scouternaplugins' ),
			array(
...
foreach ($members as $key => $value) {
	...
	foreach ($value as $key2 => $medlem) {
		if ($key2 == "unit") {
			if ($avdelningsnamn == ScouternaPlugins_ScoutnetWidgets_FixThatString($medlem['value'])) {
				if (!empty($value['unit_role']['value'])) {
					switch($r_settings) {
						case 2:
							$ledare[] = $value['first_name']['value']." ".$value['last_name']['value']."<br><i>".$value['unit_role']['value']."</i><br>tel: ".$value['contact_mobile_phone']['value']."<br>E-post: <a href=\"".$value['email']['value']."\">".$value['email']['value']."</a>";
							break;
						case 1:
							$ledare[] = $value['first_name']['value']." ".$value['last_name']['value']."<br><i>".$value['unit_role']['value']."</i><br>tel: ".$value['contact_mobile_phone']['value'];
							break;
						case 0:
						default:
...
add_action( 'widgets_init', function(){
     register_widget( 'ScouternaPlugins_ScoutnetWidgets_LedarepaAvdelning_Widget' );
});
